...
# MOTU M4 Dynamic Optimizer - Tray Module
...
tray_get_state_file() {
    echo "${TRAY_STATE_FILE:-/var/run/motu-m4-tray-state}"
}
...
tray_notify_xrun() {
    ...
    if [ "$xrun_count" -gt 0 ]; then
        tray_notify "MOTU M4 Optimizer" "$xrun_count Xruns in den letzten 30 Sekunden" "dialog-warning"
    fi
}
...
tray_get_icon_for_state() {
    local state="$1"
    local icon_dir="${TRAY_ICON_DIR:-/usr/share/icons/motu-m4}"
